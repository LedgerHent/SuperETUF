...
import weixin.common.util.BeanUtils;
import weixin.mp.api.WxMpConfigStorage;
import weixin.mp.api.WxMpPayService;
import weixin.mp.api.WxMpService;
import weixin.mp.bean.pay.request.*;
import weixin.mp.bean.pay.result.*;
  ...
  private String executeRequest(String url, String requestStr) throws WxErrorException {
    ...
    try (CloseableHttpClient httpclient = HttpClients.custom().build()) {
      ...
      try (CloseableHttpResponse response = httpclient.execute(httpPost)) {
        ...
        this.log.debug("\n[URL]:  "+url+"\n[PARAMS]: {}\n[RESPONSE]: {}", requestStr, result);
        return result;
      }
    } catch (IOException e) {
    	//this.log.error("\n[URL]:  {}\n[PARAMS]: {}\n[EXCEPTION]: {}", url, requestStr, e.getMessage());
    	this.log.error("\n[URL]:  "+url+"\n[PARAMS]: {}\n[EXCEPTION]: {}", requestStr, e.getMessage());
